...
export default function LocationMapPreview() {
  return (
    <section style={{ padding: '96px 0', backgroundColor: '#0F1629' }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '64px', alignItems: 'center' }}>
          ...
          >
            ...
            <h2 style={{ fontSize: 'clamp(26px, 4vw, 38px)', fontWeight: 800, color: '#F1F5F9', marginBottom: '20px', lineHeight: 1.3 }}>
              서울 입지 점수 지도
            </h2>
            <p style={{ fontSize: '15px', color: '#CBD5E1', lineHeight: 1.9, marginBottom: '32px' }}>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <span style={{ fontSize: '13px', fontWeight: 600, color: '#CBD5E1' }}>이달의 서울 TOP 5 입지</span>
              <span style={{ fontSize: '12px', color: '#94A3B8' }}>점수 기준</span>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {TOP_LOCATIONS.map((location, index) => (
                ...
                >
                  {/* 순위 */}
                  <span style={{ width: '28px', height: '28px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 700, flexShrink: 0, backgroundColor: index === 0 ? 'rgba(59,130,246,0.2)' : 'rgba(255,255,255,0.07)', color: index === 0 ? '#3B82F6' : '#94A3B8' }}>
                    {index + 1}
                  </span>

                  {/* 지역 */}
                  <div style={{ flex: 1 }}>
                    <span style={{ fontWeight: 600, fontSize: '14px', color: '#F1F5F9', marginRight: '8px' }}>{location.name}</span>
                    <span style={{ fontSize: '12px', color: '#94A3B8' }}>{location.district}</span>
                  </div>
