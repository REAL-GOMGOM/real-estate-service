  ...
  return (
    <ResponsiveContainer width="100%" height={350}>
      <ComposedChart data={chartData} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
        ...
        />
        <Legend wrapperStyle={{ fontSize: '12px' }} />
        <Line yAxisId="price" type="monotone" dataKey="price" stroke="#1B4DDB" strokeWidth={2} dot={{ r: 3 }} name="평균가(억)" />
        <Bar yAxisId="count" dataKey="count" fill="#6FC08A" opacity={0.3} barSize={12} name="거래건수" />
      </ComposedChart>
    </ResponsiveContainer>
  );
}
