...
function ReportCard({ report }: { report: PublicFieldReport }) {
  ...
  // Published DTOs contain the server-resolved master ID. Do not turn the
  // unverified price/date into an official tx key or fall back to a name search.
  const comparisonUrl = new URL(buildTransactionShareUrl({
    origin: 'https://relative.invalid',
    apartment: {
      masterId: report.apartmentId,
      name: report.apartmentName,
      district: report.sigungu,
      dong: report.dong,
    },
    months: 2,
    dealType: report.tradeType === 'sale' ? 'buy' : report.tradeType,
  }));
  ...
  return (
    <article className={styles.reportCard} aria-label={`${report.apartmentName} ${TRADE_LABELS[report.tradeType]} 미확인 제보`}>
      ...
      <Link
        href={`${comparisonUrl.pathname}${comparisonUrl.search}`}
        className={styles.compareLink}
        aria-label={`${report.apartmentName} ${report.sigungu}${report.dong ? ` ${report.dong}` : ''} 같은 단지 공식 실거래 비교`}
      >
        같은 단지 공식 실거래 비교<ArrowRight size={13} aria-hidden="true" />
      </Link>
...
export default function FieldReportsHome({ expanded = false }: { expanded?: boolean }) {
  ...
  const compact = !expanded && !formOpen && (feed.status !== 'ok' || feed.reports.length === 0);
  ...
  return (
    <section className={`${styles.section}${compact ? ` ${styles.compact}` : ''}`} aria-labelledby={`${prefix}-heading`}>
      <div className={styles.inner}>
        {expanded ? <Link href="/" className={styles.homeLink}>← 내집 홈</Link> : null}
        <div className={styles.sectionHeader}>
          <div>
            {!compact ? <div className={styles.eyebrow}><MessageSquareText size={15} aria-hidden="true" />이웃이 전하는 계약 소식</div> : null}
            <Heading id={`${prefix}-heading`} className={styles.title}>현장 제보가격</Heading>
            {!compact ? <p className={styles.description}>공식 실거래 공개 전, 이웃이 전하는 계약 소식</p> : null}
          </div>
        ...
        <div className={styles.disclaimer}>
          ...
          <p><strong>공식 실거래와 별개인 미확인 정보입니다.</strong>{' '}
            {compact ? '검수 후 공개 · 진위 미보증 · 공식 통계 미반영' : '검수 후 공개되지만 계약 사실·가격의 진위를 보증하지 않습니다. 공식 실거래 통계에는 반영하지 않습니다.'}
          </p>
        </div>
        ...
        {feed.status === 'loading' ? (
          <div className={styles.statusPanel} role="status" aria-busy="true"><p>현장 제보가격을 불러오는 중입니다.</p>{!compact ? <div className={styles.skeleton} aria-hidden="true" /> : null}</div>
        ...
        {compact ? <details className={styles.compactInfo}><summary>제보 이용 안내</summary>{usageInfo}</details> : usageInfo}
      </div>
    </section>
  );
}
