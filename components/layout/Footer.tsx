...
export default function Footer() {
  return (
    <footer style={{ backgroundColor: '#0A0E1A', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '64px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '48px' }}>
          ...
          {Object.entries(FOOTER_LINKS).map(([title, links]) => (
            <div key={title}>
              <h4 style={{ fontSize: '14px', fontWeight: 600, color: '#F1F5F9', marginBottom: '20px' }}>{title}</h4>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '14px' }}>
                {links.map((link) => (
                  <li key={link.href}>
                    <Link href={link.href} style={{ fontSize: '14px', color: '#64748B', textDecoration: 'none' }}>
                      {link.label}
                    </Link>
                  </li>
                ))}
        ...
        <div style={{ marginTop: '48px', paddingTop: '32px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
          <p style={{ fontSize: '12px', color: '#475569' }}>© 2026 부동산 인사이트. All rights reserved.</p>
          <p style={{ fontSize: '12px', color: '#475569' }}>데이터 기준: 국토교통부 실거래가 공개시스템</p>
        </div>
      </div>
    </footer>
  );
