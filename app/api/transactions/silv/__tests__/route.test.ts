...
describe('GET /api/transactions/silv partial contract', () => {
  ...
  it('이름·동 검색 total은 matching 거래만 세고 지역 목록 limit/최근10건 축약 전 합계를 유지한다', async () => {
    const xml = Array.from({ length: 37 }, (_, index) => XML
      .replace('분양단지', index < 35 ? '다른단지' : '가람')
      .replace('삼성동', '일원동')
      .replace('<floor>10', `<floor>${index + 1}`)).join('');
    mocks.fetchSilvMonthAllPages.mockImplementation((_key, _lawdCd, month) => Promise.resolve(month === '202607' ? xml : ''));

    const searched = await GET(new NextRequest('http://localhost/api/transactions/silv?district=강남구&q=가람&aptDong=일원동&months=2'));
    ...
    const all = await GET(new NextRequest('http://localhost/api/transactions/silv?district=강남구&months=2'));
    expect(await all.json()).toMatchObject({ total: 37 });
    const limited = await GET(new NextRequest('http://localhost/api/transactions/silv?district=강남구&months=2&limit=1'));
