...
- 카드·관리자 분류·동의문의 명칭은 `현장 제보`다. 공식 실거래와의 구분 및 진위 미보증 안내는 유지한다. 공개 카드의 작은 `신고` 버튼은 유지하며, 실제 `신고 접수` 상태는 관리자 화면에서만 표시한다(신고 폼의 제출 버튼/결과 문구와 구별).
...
### 2026-09-06 관리자 재게시 수정 및 실제 2건 공개

- 신규 접수는 `pending` 유지. 자동 게시 변경은 반영하지 않았다. 누락된 nullable 신고 필드를 실제 신고로 표시하던 문제를 수정하고, 미신고·미만료 숨김 제보만 관리자 `다시 게시`로 복원하도록 했다. 반려→숨김→게시 전이는 차단했다.
- 코드 `a7cbb4a`: 전체 198개 파일/1,915개 테스트, 전체 ESLint, TypeScript, production 빌드 통과. 로컬 실제 Redis/Lua 실행 환경은 없어 단위 테스트는 mock/스크립트 계약 검사이며, 모든 Lua 상태 분기를 실환경에서 재검증한 것은 아니다.
- 배포 `dpl_AvoWbAwEwHeYYJcsQB9s7d4tZfJj`를 운영 도메인 연결 전 점검했다. 공개 API 정상, 비로그인 관리자는 `/admin/login`으로 이동, 이후 `www.naezipkorea.com`에 승격했다. 이전 배포는 `dpl_G2yB8YcNLibapZUTAAqgUMsEwwsC`다.
- 사용자 관리자 로그인 및 정확한 2건 공개 요청에 따라, 산성역포레스티아(76.03㎡/142,500만원/2026-09-04 계약)와 미사강변 센텀팰리스(84.95㎡/125,000만원/2026-09-04 계약)를 각각 기존 인증된 관리자 액션으로 재게시했다. 원래 공개 만료일을 유지했고 다른 기록·신고 정보는 변경하지 않았다.
- 2026-09-06 20:44 KST: 관리자 `게시 중` 2건, 공개 API HTTP 200/`ok`/정확히 2건 및 금액·면적·계약일 일치, 메인과 `/field-reports` 카드 노출을 확인했다. 해당 배포 최근 1시간 서버 error/fatal 조회 결과는 0건이다. 가상 운영 제보는 생성하지 않았다.
- 브라우저 메인 이동·새로고침에서 React hydration 경고 #418이 재현됐다(20:44~20:46 KST 총 3건). 화면은 복구되어 두 제보가 표시됐고 공개 목록 이동에서는 새 오류가 없었다. 이번 배포는 홈/공개 제보 컴포넌트를 변경하지 않았으며, 공개 데이터는 초기 동일한 loading 렌더 이후 effect로 읽는다. 발생 원인은 확정하지 않았고 별도 후속 점검으로 남겼다. 따라서 브라우저 오류 전체 0건으로 보고하지 않는다.
